...
echo "QHP_NAMESPACE = \"com.skycoder42.qtapng.$verTag\"" >> Doxyfile
echo "QHP_CUST_FILTER_NAME = \"Apng $version\"" >> Doxyfile
echo "QHP_CUST_FILTER_ATTRS = \"qtapng $version\"" >> Doxyfile
echo "QHG_LOCATION = \"$qtBins/qhelpgenerator\"" >> Doxyfile
echo "INCLUDE_PATH += \"$qtHeaders\"" >> Doxyfile
echo "GENERATE_TAGFILE = \"$destDir/qtapng/qtapng.tags\"" >> Doxyfile
if [ "$DOXY_STYLE" ]; then
...
for tagFile in $(find "$qtDocs" -name *.tags); do
	if [ $(basename "$tagFile") != "qtapng.tags" ]; then

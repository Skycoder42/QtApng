...
:: get patch tool
powershell -Command "Invoke-WebRequest http://downloads.sourceforge.net/project/gnuwin32/patch/2.5.9-7/patch-2.5.9-7-bin.zip -MaximumRedirection 10 -OutFile patch.zip" || exit /B 1
dir
...
:: get zlib
powershell -Command "Invoke-WebRequest http://www.zlib.net/zlib%ZLIB_VERSION_P%.zip -OutFile zlib.zip" || exit /B 1
...
:: get libpng
powershell -Command "Invoke-WebRequest http://prdownloads.sourceforge.net/libpng/lpng%LIBPNG_VERSION_P%.zip?download -OutFile libpng.zip" || exit /B 1
...
:: get apng patch
powershell -Command "Invoke-WebRequest http://downloads.sourceforge.net/sourceforge/libpng-apng/libpng-%LIBPNG_VERSION%-apng.patch.gz -OutFile libpng-apng.patch.gz" || exit /B 1
